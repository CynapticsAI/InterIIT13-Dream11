...
def train_test_model(train_date1, train_date2, test_date1, test_date2):
    """For the training and the testing of the model accoring to the dates"""
    train_df = df[(df['date'] >= pd.to_datetime(train_date1)) &
                  (df['date'] <= pd.to_datetime(train_date2))]
    test_df = df[(df['date'] >= pd.to_datetime(test_date1)) &
                 (df['date'] <= pd.to_datetime(test_date2))]
    train_df.to_csv(f"../data/processed/training_data_{train_date2}.csv", index=False)
    ...
    # Save the model
    torch.save(m.state_dict(), f"../model_artifacts/model_{train_date2}.pt")
    ...
    for i, (input_batch, output_batch) in enumerate(zip(test_dataloader, unnorm_dataloader)):
        ...
        assert predicted_score <= actual_score
        mae += abs(actual_score - predicted_score)
        mape += abs((actual_score - predicted_score)*100 / actual_score)
        print(f"Predicted: {predicted_score}\tActual: {actual_score}")
        if i == 0:
            print(f"Predicted: {predicted_score}\tActual: {actual_score}")
...
def model_train_and_test_dates(date1, date2):
    """Filter the data according to the dates provided"""
...
st.title("Model_UI")
st.subheader("Training_Data Time Period")

train_date1 = datetime(2000, 1, 1)
train_date2 = datetime(2000, 1, 1)
test_date1 = datetime(2000, 1, 1)
test_date2 = datetime(2000, 1, 1)

col1, col2 = st.columns(2)

with col1:
    train_date1 = st.date_input(
        label="Start Date",
...
with col2:
    train_date2 = st.date_input(
        label="End Date",
        value=datetime(2020, 12, 31),
        min_value=datetime(2000, 1, 1),
        max_value=datetime(2025, 12, 31),
        format="YYYY-MM-DD",
        help="Select the end date for filtering the data."
    )

if st.button("Filtered Train Data"):
    filtered_train_data = model_train_and_test_dates(train_date1, train_date2)

    if not filtered_train_data.empty:
        st.write("### Filtered Data:")
        combined_data = pd.concat(
            [filtered_train_data.head(1), filtered_train_data.tail(1)])
        st.dataframe(combined_data)

        csv = filtered_train_data.to_csv(index=False)
...
st.subheader("Testing_Data Time Period")

col1, col2 = st.columns(2)

with col1:
    test_date1 = st.date_input(
        label="Start Date",
...
if st.button("Filtered Test Data"):
    filtered_test_data = model_train_and_test_dates(test_date1, test_date2)

    if not filtered_test_data.empty:
        st.write("### Filtered Data:")
        combined_data = pd.concat(
            [filtered_test_data.head(1), filtered_test_data.tail(1)])
        st.dataframe(combined_data)

        csv = filtered_test_data.to_csv(index=False)
...
if st.button("Train and Test"):
    mae_final, mape_final = train_test_model(
        train_date1, train_date2, test_date1, test_date2)
    # print(type(mae_final), mae_final)
    st.write("## MAE: ", str(mae_final))
    st.write("## MAPE: ", str(mape_final))
